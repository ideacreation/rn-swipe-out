...
class Swipeout extends React.Component {
  ...
  panResponderObject = {
    ...
    onPanResponderMove: (event, gestureState) => {
      if (this.isSwipingVertically(gestureState)) return;
  ...
  isSwipingVertically = gestureState => {
    const { dx, dy } = gestureState;
    const absDx = Math.abs(dx);
    const absDy = Math.abs(dy);
    const angle = Math.atan2(absDy, absDx);

    return absDy >= absDx || angle > Math.PI / 12;
  };
  ...
  onPanResponderEndTrans = (event, gestureState) => {
    ...
    if (this.isSwipingVertically(gestureState)) {
    ...
    if (
      this.panAnimValue <= this.state.leftActionThreshold &&
      ...
      <Animated.View
        style={[
          styles.optionWrapper,
          {
            ...
            transform: [
              {
                translateX: this.panAnim.interpolate(
                  this.state.interpolation.left
                )
              }
            ]
          }
        ...
        style={[
          styles.optionWrapper,
          {
            ...
            transform: [
              {
                translateX: this.panAnim.interpolate(
                  this.state.interpolation.right
                )
              }
            ]
          }
  ...
  renderCommonOption = (option, index) => {
    ...
    if (typeof option.component === "function") {
      return (
        <option.component
          key={index}
          hide={() => this.performAnimation(0)}
          animatedValue={this.panAnim}
        />
      );
    }
    return (
      <Touchable
        ...
        key={index}
      >
        <Text style={[styles.text, filterBackGroundColorStyle]}>
          {option.text}
        </Text>
      </Touchable>
    );
        ...
        style={[
          styles.flexGrow,
          {
            transform: [
              {
                translateX:
                  this.state.left.length === 0 && this.state.right.length === 0
                    ? 0
                    : this.panAnim.interpolate(
                        this.state.interpolation.children
                      )
              }
            ]
          }
    ...
    return (
      <View style={[styles.container, filterSelfStyle(this.props.style)]}>
        {this.state.left.length > 0
          ? this.renderLeftOptions(this.state.left)
          : null}
        {this.state.right.length > 0
          ? this.renderRightOptions(this.state.right)
          : null}
