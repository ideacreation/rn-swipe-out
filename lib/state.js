import { toArray, decaySeries, uniqueSortArray } from "./util";
...
export function setActionThreshold(state, props) {
  ...
  const leftThresholdDistance =
    leftThreshold <= 1 ? state.leftWidth * leftThreshold : leftThreshold;
  const rightThresholdDistance =
    rightThreshold <= 1 ? state.rightWidth * rightThreshold : rightThreshold;
...
function _buildInterpolate(leftWidth, rightWidth) {
  ...
  const rightDecaySeriesInputRange = decaySeries(rightWidth, 30, 1)
    .map(v => -v)
    .sort((v1, v2) => v1 - v2);
  const rightDecaySeriesOutputRange = decaySeries(rightWidth, 30, 0.77)
    .map(v => -v)
    .sort((v1, v2) => v1 - v2);
  const childrenInputRange = [
    ...rightDecaySeriesInputRange,
    0,
    ...leftDecaySeriesInputRange
  ];
  const childrenOutputRange = [
    ...rightDecaySeriesOutputRange,
    0,
    ...leftDecaySeriesOutputRange
  ];
